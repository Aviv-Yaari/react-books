...
const storageBooks = storageService.loadFromStorage('books');

let gBooks = storageBooks || [
  {
    id: 'OXeMG8wNskc',
...
let gSearchCache = storageService.loadFromStorage('add-search') || [];
...
function getNegBookId(bookId, diff) {
  ...
  let resIdx = idx + diff;
  if (resIdx === -1) resIdx = gBooks.length - 1;
  else if (resIdx === gBooks.length) resIdx = 0;
  return gBooks[resIdx].id;
}
...
function searchInAPI(search) {
  const searchInCache = gSearchCache.find((item) => Object.keys(item)[0] === search);
  if (searchInCache) {
  ...
  return axios.get(url).then((res) => {
    ...
    gSearchCache.push({ [search]: mappedRes });
    storageService.saveToStorage('add-search', gSearchCache);
    return mappedRes;
  });
}
...
function mapAPIRes(items) {
  return items.map((item) => {
    return {
      ...
      subtitle: item.volumeInfo.description || 'Unknown',
